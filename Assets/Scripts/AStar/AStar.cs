...
public class AStar{
    ...
    private static bool GenerateMap(Point s,Point e)
    {
        ...
        MAX_PNT = new Point(GameManager.Instance.mapwidth, GameManager.Instance.mapHeight);
        START_PNT = s;
        END_PNT = e;
        ...
        for (int i = 0; i < MAX_PNT.x; i++)
        {
            ...
            {
                if (GameManager.Instance.elements[i, j].elementContent == ElementContent.door ||
                    GameManager.Instance.elements[i, j].elementContent == ElementContent.enemy ||
                    GameManager.Instance.elements[i, j].elementContent == ElementContent.bigWall ||
                    GameManager.Instance.elements[i, j].elementContent == ElementContent.smallWall ||
                    GameManager.Instance.elements[i, j].elementState == ElementState.marked ||
                    (GameManager.Instance.elements[i, j].elementContent == ElementContent.trap && GameManager.Instance.elements[i, j].elementState == ElementState.uncovered))
                {
                    MAP[i, j] = WALL;
                }
                else if (GameManager.Instance.elements[i, j].elementState == ElementState.uncovered ||
                    (GameManager.Instance.elements[i, j].elementContent == ElementContent.tool && ((ToolElement)GameManager.Instance.elements[i, j]).hide == false) ||
                    (GameManager.Instance.elements[i, j].elementContent == ElementContent.gold && ((GoldElement)GameManager.Instance.elements[i, j]).hide == false))
                {
                    MAP[i, j] = SPACE0;
                }
                else
    ...
    private static double HManhattanDistance(Point pnt)
    {
        ...
        {
            return Math.Abs(pnt.x - END_PNT.x) + Math.Abs(pnt.y - END_PNT.y) + (GameManager.Instance.mapwidth * 100);
        }
    }
